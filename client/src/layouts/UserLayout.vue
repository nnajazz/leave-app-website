  ...
  <div class="flex h-screen bg-[#f8fbff]">
    <aside class="w-64 bg-white p-6 shadow-xl border-r border-gray-200 flex flex-col">
      ...
      <nav class="space-y-2 flex-grow">
        <RouterLink 
          to="/user/dashboard" 
          active-class="bg-[#5e77ff] text-white" 
        ...
        >
          <img :src="IconDashboard" class="w-5 h-5 mr-3" 
               :class="{'invert': $route.path.startsWith('/user/dashboard')}" 
               alt="Dashboard Icon" />
          Dashboard
        </RouterLink>
        ...
        <RouterLink 
          ...
          class="flex items-center p-3 rounded-xl font-semibold transition duration-150 text-gray-600 hover:bg-gray-100"
        >
          <img :src="IconList" class="w-5 h-5 mr-3" 
               :class="{'invert': $route.path.startsWith('/user/history')}" 
               alt="History Icon" />
          History
        </RouterLink>
        ...
        <RouterLink 
          ...
          class="flex items-center p-3 rounded-xl font-semibold transition duration-150 text-gray-600 hover:bg-gray-100"
        >
          <img :src="IconMandatory" class="w-5 h-5 mr-3" 
               :class="{'invert': $route.path.startsWith('/user/mandatory')}" 
               alt="Mandatory Icon" />
          Mandatory
        </RouterLink>

        <RouterLink 
          to="/user/adjust-history" 
          active-class="bg-[#5e77ff] text-white" 
          class="flex items-center p-3 rounded-xl font-semibold transition duration-150 text-gray-600 hover:bg-gray-100"
        >
          <img :src="IconAdjustHistory" class="w-5 h-5 mr-3" 
               :class="{'invert': $route.path.startsWith('/user/adjust-history')}" 
               alt="Adjust History Icon" />
          Adjust History
        </RouterLink>
        ...
        <RouterLink 
          ...
          class="flex items-center p-3 rounded-xl font-semibold transition duration-150 text-gray-600 hover:bg-gray-100"
        >
          <img :src="IconInformation" class="w-5 h-5 mr-3" 
               :class="{'invert': $route.path.startsWith('/user/information')}" 
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
      <header class="flex justify-between items-center p-8 pb-4">
        ...
        <div class="flex items-center space-x-4">
          ...
          >
            <img :src="IconRoleAdmin" class="w-5 h-5" alt="Admin Side Icon" />
          ...
          >
            <img :src="IconLogout" 
                 class="w-6 h-6" 
                 alt="Logout Icon" 
                 :class="{'invert': isHoveringLogout}" 
                 @mouseover="isHoveringLogout = true" 
                 @mouseout="isHoveringLogout = false" />
            <span>Logout</span>
          </button>
        </div>
      </header>
    ...
    >
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-xl mx-4" @click.stop>
        ...
        <div class="p-6 border-t border-gray-200 flex justify-end space-x-3">
          <button 
            @click="isSettingsModalOpen = false" 
            class="px-4 py-2 text-sm font-semibold rounded-xl text-gray-600 hover:bg-gray-100 transition duration-150"
          >
            Cancel
          </button>
          <button 
            class="px-4 py-2 text-sm font-semibold rounded-xl text-white bg-green-500 hover:bg-green-600 transition duration-150"
          >
            Save Change
          </button>
        </div>
